...
export class OwlSalesDashboard extends Component {
    ...
    async onChangePeriod(){
        ...
        //console.log(this.state.period, moment().subtract(this.state.period, 'days').format('L'))
        //console.log(this.state.period, moment().subtract(this.state.period, 'days').format('DD/MM/YYYY'))
        this.getDates()
        await this.getQuotations()
        await this.getOrders()
    ...
    async getQuotations(){
        let domain = [['state', 'in', ['quotation_sent', 'quotation']]]; // Changed state values to match your custom model
        if (this.state.period > 0){
            domain.push(['date', '>', this.state.current_date]); // Changed field name to match your custom model
        }
        const data = await this.orm.searchCount("liquor_store.sales.order", domain); // Changed model name to match your custom model
        this.state.quotations.value = data;
    
        // previous period
        let prev_domain = [['state', 'in', ['quotation_sent', 'quotation']]]; // Changed state values to match your custom model
        if (this.state.period > 0){
            prev_domain.push(['date', '>', this.state.previous_date], ['date', '<=', this.state.current_date]); // Changed field name to match your custom model
        }
        const prev_data = await this.orm.searchCount("liquor_store.sales.order", prev_domain); // Changed model name to match your custom model
    ...
    async getOrders(){
        // orders
        let domain = [['state', 'in', ['sales_order', 'done']]]; // Changed state values to match your custom model
        if (this.state.period > 0){
            let currentDate = moment(this.state.current_date, 'MM/DD/YYYY').format('YYYY-MM-DD'); // Convert current_date to 'YYYY-MM-DD' format
            domain.push(['date', '>', currentDate]); // Changed field name to match your custom model
        }
        const data = await this.orm.searchCount("liquor_store.sales.order", domain); // Changed model name to match your custom model
        //this.state.quotations.value = data

        // previous period
        let prev_domain = [['state', 'in', ['sales_order', 'done']]]; // Changed state values to match your custom model
        if (this.state.period > 0){
            let previousDate = moment(this.state.previous_date, 'MM/DD/YYYY').format('YYYY-MM-DD'); // Convert previous_date to 'YYYY-MM-DD' format
            let currentDate = moment(this.state.current_date, 'MM/DD/YYYY').format('YYYY-MM-DD'); // Convert current_date to 'YYYY-MM-DD' format
            prev_domain.push(['date', '>', previousDate], ['date', '<=', currentDate]); // Changed field name to match your custom model
        }
        const prev_data = await this.orm.searchCount("liquor_store.sales.order", prev_domain); // Changed model name to match your custom model
        const percentage = ((data - prev_data)/prev_data) * 100;
